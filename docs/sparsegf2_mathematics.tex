...
(graph-defined monitored Clifford circuits). Part~III collects shorter notes on further topics: the Bravyi--Maslov
sampler, batched simulation, and finite-size-scaling analysis.
\end{abstract}
...
\label{sec:sampling}
$\texttt{random\_symplectic}(n,\mathrm{rng})$ samples uniformly from
$\Sp(2n,\F_2)$ by realizing the \emph{same} greedy construction with uniform
choices at each step (the incremental kernel-basis sampler). It maintains the
...
\end{proof}
The cost is $O(n^3)$ bit-operations (a kernel computation per step). For $n=2$
the routine short-circuits to a uniform draw from the cached $\Sp(4,\F_2)$ table
so the RNG state advances identically across entry points.
\begin{remark}
The asymptotically faster Bravyi--Maslov $O(n^2)$ canonical-form
sampler~\cite{BM} is not used; the kernel-basis method is simpler to verify and
adequate for the $n\lesssim 200$ regime targeted here
...
then its $\Delta$ color classes are perfect matchings. The two implemented
families (for even $|V|=n$):
\begin{itemize}[leftmargin=1.5em]
...
\end{itemize}
For odd $n$ neither has a perfect matching, and brickwork gating is rejected at
configuration time. Graphs are stored with their edge list, a canonical
$1$-factorization (when one exists), a uniform perfect-matching sampler, and a
graph6 string.
...
\begin{itemize}[leftmargin=1.5em]
\item \textbf{Brickwork:} a whole perfect matching ($n/2$ gates). The matching
  is chosen by the matching mode:
  \emph{round\_robin} (cycle through the $1$-factorization, layer $t$ uses
  matching $t\bmod\chi'$), \emph{palette} (uniform over the $\chi'$ matchings of
  the fixed $1$-factorization), or \emph{fresh} (uniform over \emph{all} perfect
...
for $O(n)$ depth, or $\texttt{depth\_factor}\cdot\lceil\log_2 n\rceil$ for
$O(\log n)$; the \texttt{until\_purified} mode currently uses the $O(n)$ value as
a fixed depth cap (the early-termination behavior it anticipates is a roadmap
item, Part~III). A brickwork layer fires $n/2$ gates and touches every qubit once,
so $T_{\mathrm{base}}$ \emph{is} its gates-per-qubit budget. A random\_edge layer
...
\end{equation}
layers for the same budget. Thus single-edge random\_edge ($m=1$) runs
$O(n^2)$ layers; $m=n/2$ matches brickwork. This is the precise sense in which
the three gating styles apply the \emph{same} total gate count at equal
\texttt{depth\_factor}.

\subsection{Expected gate-to-measurement ratio}
With $e_g$ the expected gates per layer ($e_g=n/2$ for brickwork, $e_g=m$ for
random\_edge) and $e_m$ the expected measurements per layer, the expected ratio
recorded before simulation is $r=e_g/e_m$ (or $\infty$ when $e_m=0$), where by
measurement mode
\begin{equation}
  e_m \;=\;
  \begin{cases}
    n\,p & \text{bernoulli},\\
    2\,e_g\,p & \text{gated (each gate touches 2 qubits)},\\
    2\,p & \text{random\_pair}.
  \end{cases}
\end{equation}
For the brickwork$+$bernoulli baseline this collapses to $r=\tfrac{n/2}{np}=\tfrac1{2p}$.
...
\emph{two independent} pseudorandom streams: a \emph{construction} stream
(seeded $\texttt{base\_seed}+\texttt{sample\_seed}$) that fixes the gate
placement, the Clifford indices, and the measured-qubit selection, consumed in a
fixed per-layer order; and a \emph{measurement-outcome} stream (seeded
independently on the simulator instance) that supplies the phase-free coins. The
streams are decoupled so a fixed circuit can be replayed with different outcomes.
...
\emph{Shorter notes on additional topics.}

\section{Bravyi--Maslov \texorpdfstring{$O(n^2)$}{O(n^2)} sampling}
\label{sec:roadmap-bm}
Canonical decomposition $C=F_1HSF_2$ with Mallows-distributed permutation, for
faster uniform $\Sp(2n,\F_2)$ sampling at large $n$.

\section{Koenig--Smolin integer indexing}
Deterministic bijection between $[\,0,|\Sp(2n,\F_2)|)$ and group elements (forward
and inverse), via symplectic transvections.
...
perfect-matching sampler.

\section{Additional measurement modes}
\texttt{uniform\_count}: a fixed number of measured qubits per layer.

\section{Adaptive depth and time series}
\texttt{until\_purified} early termination (stop when the order parameter
vanishes) and per-layer order-parameter recording. The \texttt{until\_purified}
depth mode and the \texttt{record\_time\_series} flag are already accepted and
validated by the configuration, but the runner does not yet act on either (the
former currently behaves as a fixed $O(n)$ cap; see Section~\ref{sec:depth}).
...
scaling to locate the critical point $p_c$ and extract critical exponents.
