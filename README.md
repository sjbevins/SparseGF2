...
| `analysis` | `graph` + `data` + `parallel` + `progress` | the full analysis layer |
...
The full public API is the `__all__` of
[`src/sparsegf2/__init__.py`](src/sparsegf2/__init__.py).
...
Every gating, matching (1-factorization), and measurement mode has a rendered
example under [docs/figures/gallery/](docs/figures/gallery/), regenerated by
`python docs/figures/circuit_gallery.py`. The matching modes pair with a uniform
measurement set, since when a brickwork layer gates every qubit the `gated`
candidate set is identical to measuring everything.
...
A stabilizer state on $n$ qubits is fixed by $2n$ generators, each a Pauli
string written as a row $[x \mid z]$ over $\mathbb{F}_2$ (the symplectic part).
SparseGF2 stores this tableau **sparsely**: for each generator it keeps the list
of qubits it acts on, and for each qubit it keeps the list of generators that
touch it (an inverted index). A two-qubit gate or a $Z$-measurement only needs
the generators supported on the qubits involved, which the inverted index
returns in $O(1)$. A measurement is then a rank update whose cost is
